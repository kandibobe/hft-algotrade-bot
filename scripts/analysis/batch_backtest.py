...
class BatchBacktester:
    ...
    def run_single_backtest(self, strategy: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "full_output": output
                if not success
                else "",  # Сохраняем полный вывод только при ошибках
                "command": " ".join(cmd),
            }
    ...
    def print_summary(self):
        ...
        print(
            f"Эффективность параллелизации: {total_duration / max(total_duration, 1) * self.max_workers:.1f}x"
        )
        print("=" * 70)
...
def main():
    ...
    parser.add_argument("--timeframe", type=str, default="5m", help="Таймфрейм (по умолчанию: 5m)")
    parser.add_argument(
        "--no-docker", action="store_true", help="Использовать локальный freqtrade вместо Docker"
    )
    parser.add_argument(
        "--max-workers",
        type=int,
        help="Максимальное количество параллельных процессов (по умолчанию: min(количество стратегий, 3))",
    )
    parser.add_argument(
        "--timeout",
        type=int,
    ...
    parser.add_argument("--config", type=str, help="Путь к конфигурационному файлу freqtrade")
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Проверить конфигурацию без запуска тестов"
    )
