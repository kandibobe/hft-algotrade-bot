...
class SimpleWalkForwardTest:
    ...
    def run_test(
        self, pair: str = "BTC/USDT", timeframe: str = "5m", train_size: float = 0.7
    ) -> dict[str, Any]:
        ...
        logger.info(
            f"Positive labels in train: {y_train.sum()}/{len(y_train)} ({y_train.mean():.1%})"
        )
    ...
    def simulate_trading(
        self, test_df: pd.DataFrame, y_true: pd.Series, y_pred_proba: pd.Series
    ) -> dict[str, Any]:
    ...
    def create_visualization(
        self, results: dict[str, Any], test_df: pd.DataFrame, y_pred_proba: pd.Series
    ) -> None:
        ...
        if buy_signals.any():
            ...
            ax1.scatter(buy_points, buy_prices, color="green", label="Buy Signal", alpha=0.6, s=50)
...
def main():
    """Основная функция."""
    parser = argparse.ArgumentParser(description="Simple Walk-Forward Test for ML Trading Strategy")
    parser.add_argument(
    ...
    parser.add_argument(
        "--train-size", type=float, default=0.7, help="Training set size ratio (0.0-1.0)"
    )
    ...
    try:
        ...
        results = test.run_test(
            pair=args.pair, timeframe=args.timeframe, train_size=args.train_size
        )
