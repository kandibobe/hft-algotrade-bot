...
class WalkForwardOptimizer:
    ...
    def generate_windows(self, start_date: datetime, end_date: datetime) -> list[WalkForwardWindow]:
        ...
        while (
            current_date + timedelta(days=30 * (self.train_months + self.test_months)) <= end_date
        ):
        ...
        if sharpe is not None:
            try:
                ...
                if sharpe_val < self.min_sharpe:
                    reasons.append(f"Sharpe Ratio {sharpe_val:.2f} < {self.min_sharpe}")
            except:
        ...
        if drawdown:
            try:
                ...
                if abs(dd_val) > self.max_drawdown:
                    reasons.append(f"Max Drawdown {abs(dd_val):.2%} > {self.max_drawdown:.2%}")
            except:
    ...
    def _print_summary(self, results_df: pd.DataFrame):
        ...
        # Save results
        output_file = (
            f"walk_forward_results_{self.strategy}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
...
def main():
    ...
    results = optimizer.run_walk_forward(start_date=args.start_date, end_date=args.end_date)
