...
Implements sliding window validation using core system components.
Ensures consistency with the training pipeline and applies critical MFT fixes.

1. Load data
2. Split into sliding windows (Train -> Test)
3. For each window:
   - Engineer features (using src.ml.training.feature_engineering)
   - Generate labels (using src.ml.training.labeling with Phantom Execution Fix)
...
class WalkForwardAnalysis:
    ...
    def __init__(
        ...
        # Feature Engineer handles scaling internally
        self.feature_config = FeatureConfig(
            scale_features=True, scaling_method="standard", remove_correlated=True
        )
        self.label_config = TripleBarrierConfig(
            take_profit=0.008, stop_loss=0.004, max_holding_period=48
        )
    ...
    def create_windows(
        ...
        if train_candles + test_candles > n:
            ...
            logger.warning(
                f"Adjusting window sizes due to limited data: train={train_candles}, test={test_candles}"
            )
    ...
    def train_model(
        ...
        if use_class_weight and len(unique_classes) > 1:
            ...
            weights = compute_class_weight(
                class_weight="balanced", classes=np.array(unique_classes), y=y_train
            )
    ...
    def process_window(
        ...
        try:
            # 1. Feature Engineering (using core FeatureEngineer)
            logger.info("Creating features (Core Pipeline)...")

            # Initialize fresh feature engineer for each window to prevent leakage
            fe = FeatureEngineer(self.feature_config)

            # Fit on TRAIN, transform on TEST
            # This handles scaling and stationarity automatically
            X_train = fe.fit_transform(train_df)
            X_test = fe.transform(test_df)

            # 2. Labeling (using core TripleBarrierLabeler)
            logger.info("Generating labels (Phantom Execution Fixed)...")

            # Generate labels
            y_train_full = self.labeler.label(train_df)
            y_test_full = self.labeler.label(test_df)

            # Align features and labels (intersection of indices)
            train_idx = X_train.index.intersection(y_train_full.index)
            test_idx = X_test.index.intersection(y_test_full.index)

            X_train = X_train.loc[train_idx]
            y_train = y_train_full.loc[train_idx]

            X_test = X_test.loc[test_idx]
            y_test = y_test_full.loc[test_idx]
            ...
            model = self.train_model(
                X_train, y_train_encoded, model_type=model_type, use_class_weight=True
            )
            ...
            # Use original test_df for returns calculation
            trading_results = self.simulate_trading(
                test_df.loc[X_test.index], X_test, model, probability_threshold
            )
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Walk-Forward Analysis for ML Trading Strategies")
    parser.add_argument(
    ...
    parser.add_argument(
        "--step-days", type=int, default=30, help="Step size in days (default: 30 = 1 month)"
    )
    parser.add_argument(
        "--model",
        default="xgboost",
