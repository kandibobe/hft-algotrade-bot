...
def run_backtest(strategy: str, timerange: str) -> dict[str, Any]:
    """
...
def run_wfo(
    ...
    # Create windows
    windows = create_windows(start, end, num_windows, train_ratio, validate_ratio)
...
def save_results(results: list[WFOResult], output_dir: str, strategy: str) -> None:
    """
    ...
    with open(yaml_path, "w") as f:
        yaml.dump([asdict(r) for r in results], f, default_flow_style=False)
...
def main():
    parser = argparse.ArgumentParser(description="Walk-Forward Optimization for Stoic Citadel")
    parser.add_argument("--strategy", "-s", default="StoicEnsembleStrategy", help="Strategy name")
    parser.add_argument(
        "--timerange", "-t", default="20230101-20240101", help="Total timerange (YYYYMMDD-YYYYMMDD)"
    )
    parser.add_argument("--windows", "-w", type=int, default=6, help="Number of WFO windows")
    parser.add_argument("--train-ratio", type=float, default=0.6, help="Training data ratio")
    parser.add_argument("--validate-ratio", type=float, default=0.2, help="Validation data ratio")
    parser.add_argument("--epochs", "-e", type=int, default=100, help="Hyperopt epochs per window")
    parser.add_argument("--output", "-o", default="reports/wfo", help="Output directory")
    parser.add_argument("--dry-run", action="store_true", help="Show windows without running")
...
""")
    ...
    if args.dry_run:
        ...
        windows = create_windows(start, end, args.windows, args.train_ratio, args.validate_ratio)
