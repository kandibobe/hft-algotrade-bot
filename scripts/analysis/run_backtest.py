...
def main():
    parser = argparse.ArgumentParser(description="Quick backtest runner with profiles")

    parser.add_argument(
        "--profile", type=str, help="Use predefined profile (quick, full, aggressive, stable, all)"
    )
    ...
    parser.add_argument(
        "--no-docker", action="store_true", help="Run without Docker (use local freqtrade)"
    )
    ...
    if args.profile:
        ...
        runner.run_backtest(
            pairs=args.pair, timeframe=args.timeframe, days=args.days, strategy=args.strategy
        )
