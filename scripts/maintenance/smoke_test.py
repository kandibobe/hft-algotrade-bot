...
def test_imports() -> tuple[bool, str]:
    ...
    modules_to_test = [
        ...
        (
            "user_data.strategies.StoicEnsembleStrategyV4",
            ["StoicEnsembleStrategyV4"],
            True,
        ),  # Optional: freqtrade dependency
    ]
    ...
    for module_name, attributes, optional in modules_to_test:
        try:
            ...
            if optional:
                print_warning(f"{module_name}: {e!s} (optional)")
            else:
                all_passed = False
                error_msg = f"{module_name}: {e!s}"
                print_failure(f"{module_name}: {e!s}")
    ...
    return (
        all_passed,
        "All imports passed" if all_passed else f"Import failures: {', '.join(failures)}",
    )
...
def test_inference_service() -> tuple[bool, str]:
    ...
    try:
        from src.ml.inference_service import MLInferenceService, MLModelConfig, PredictionRequest

        # Test that classes can be instantiated
        config = MLModelConfig(
            model_name="test_model", model_path="test.pkl", feature_columns=["feature1", "feature2"]
        )
...
def test_market_regime_filter() -> tuple[bool, str]:
    ...
    try:
        ...
        config = RegimeFilterConfig(
            ema_period=200, adx_trend_threshold=25.0, allow_trade_in_sideways=False
        )
...
def test_strategy() -> tuple[bool, str]:
    ...
    try:
        # Configuration for strategy init
        config = {"pair_whitelist": [], "exchange": {"name": "binance"}}
        from user_data.strategies.StoicEnsembleStrategyV4 import StoicEnsembleStrategyV4

        # Try to create strategy instance
        strategy = StoicEnsembleStrategyV4(config=config)
...
def test_risk() -> tuple[bool, str]:
    ...
    try:
        ...
        position = calculate_position_size_fixed_risk(
            account_balance=10000, risk_per_trade=0.02, entry_price=50000, stop_loss_price=47500
        )
...
) -> tuple[int, int, list[tuple[str, bool, str, float]]]:
