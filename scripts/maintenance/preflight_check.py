...
""")
    ...
    def check_python_version(self) -> None:
        ...
        self.check("Python Version", success, f"{version} {'>= 3.10' if success else '< 3.10'}")

    def check_critical_libraries(self) -> None:
        """Check critical libraries can be imported."""
        libraries = ["freqtrade", "xgboost", "ccxt", "pandas", "tenacity"]
    ...
    def check_models_directory(self) -> None:
        ...
        if not exists:
            try:
                ...
                self.check("Models Directory", True, "Created (was missing)")
            except Exception as e:
                self.check("Models Directory", False, f"Failed to create: {e}")
        else:
            self.check("Models Directory", True, "Exists")
    ...
    def check_critical_source_files(self) -> None:
        ...
        critical_files = [
            ...
            (
                "trainer.py",
                Path("src/ml/training/trainer.py"),
                False,
            ),  # Not critical if alternative exists
    ...
    def check_docker_compose(self) -> None:
        ...
        self.check("Docker Compose", found, f"{'Found: ' + docker_file if found else 'Not found'}")
