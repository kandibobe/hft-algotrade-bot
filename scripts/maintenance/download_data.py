...
PRESETS = {
    ...
    "meme": {"description": "Meme coins", "pairs": ["DOGE/USDT", "SHIB/USDT", "PEPE/USDT"]},
    "all": {
...
def download_data(pairs: list, timeframe: str = "5m", days: int = 30, use_docker: bool = True):
    """
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print("  2. Run backtest:  python scripts/run_backtest.py --profile quick")
            print("  3. View in FreqUI: http://localhost:3000")
...
def main():
    parser = argparse.ArgumentParser(description="Download historical data from exchange")

    parser.add_argument(
        "--preset", type=str, choices=list(PRESETS.keys()), help="Use predefined pair preset"
    )
    parser.add_argument("--list-presets", action="store_true", help="Show available presets")
    parser.add_argument(
        "--pair", type=str, nargs="+", help="Trading pairs (e.g., BTC/USDT ETH/USDT)"
    )
    ...
    parser.add_argument(
        "--no-docker", action="store_true", help="Use local freqtrade instead of Docker"
    )
    ...
    # Download
    download_data(
        pairs=pairs, timeframe=args.timeframe, days=args.days, use_docker=not args.no_docker
    )
