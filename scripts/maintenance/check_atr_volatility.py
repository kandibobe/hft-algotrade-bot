...
def analyze_volatility_dynamics(pair="BTC/USDT"):
    ...
    print(
        f"Crash periods (returns < -2%): {df['crash'].sum()} ({df['crash'].sum() / len(df) * 100:.2f}%)"
    )
    ...
    atr_normal_vals = df.loc[~df["crash"], "atr_pct"].sample(
        min(len(atr_crash) * 10, len(df)), random_state=42
    )
