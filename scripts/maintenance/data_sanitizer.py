...
def clean_data(df: pd.DataFrame, timeframe: str) -> pd.DataFrame:
    ...
    full_range = pd.date_range(
        start=df_clean.index.min(), end=df_clean.index.max(), freq=f"{expected_minutes}min"
    )
    ...
    if nan_mask.any():
        ...
        logger.info(
            f"Forward-filled {filled_count} small gaps (≤ {MAX_FORWARD_FILL_MINUTES} minutes)"
        )
...
def print_report(report: dict):
    ...
    print("\n📈 DATA STATISTICS:")
    print(f"  • Raw data: {report['raw_data']['rows']:,} rows ({report['raw_data']['days']} days)")
    print(
        f"  • Cleaned data: {report['cleaned_data']['rows']:,} rows "
    ...
    print(
        f"  • Large gaps (> {MAX_ALLOWED_GAP_MINUTES} min): {report['gap_analysis']['large_gaps']}"
    )
    print(f"  • Maximum gap: {report['gap_analysis']['max_gap_minutes']:.1f} minutes")

    print("\n🧹 CLEANING RESULTS:")
    print(
        f"  • NaN values removed: {report['raw_data']['quality']['nan_count']} → "
        f"{report['cleaned_data']['quality']['nan_count']}"
    )
    print(
        f"  • Inf values removed: {report['raw_data']['quality']['inf_count']} → "
    ...
    print(
        f"  • Has 365+ days of data: {'✓' if criteria['has_365_days'] else '✗'} "
        f"({report['cleaned_data']['days']} days)"
    )
    print(f"  • No NaN/Inf values: {'✓' if criteria['no_nan_inf'] else '✗'}")
    print(
        f"  • Max gap ≤ {MAX_ALLOWED_GAP_MINUTES} min: {'✓' if criteria['max_gap_ok'] else '✗'} "
    ...
    if not criteria["no_nan_inf"]:
        print("   ⚠️  Data still contains NaN/Inf values")
    if not criteria["max_gap_ok"]:
        print("   ⚠️  Large gaps present in data")
...
def process_symbol(symbol: str, timeframe: str, output_dir: Path) -> bool:
    ...
    try:
        ...
        report = generate_report(
            symbol, timeframe, raw_df, clean_df, gap_analysis, quality_before, quality_after
        )
        print_report(report)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Clean and sanitize OHLCV data for ML training")
    parser.add_argument(
        "--symbols",
        nargs="+",
