...
class AdvancedHyperparameterOptimizer3Y:
    ...
    def optimize(
        ...
        # Run optimization
        self.study.optimize(
            objective, n_trials=n_trials, timeout=timeout, show_progress_bar=True, n_jobs=n_jobs
        )
...
def main():
    ...
    parser.add_argument(
        "--trials", type=int, default=300, help="Number of Optuna trials (default: 300)"
    )
    parser.add_argument(
        "--timeout", type=int, default=None, help="Optimization timeout in seconds (default: None)"
    )
    parser.add_argument(
        "--quick", action="store_true", help="Quick mode (use only recent data for testing)"
    )
    parser.add_argument(
        "--n_jobs", type=int, default=-1, help="Number of parallel jobs (-1 for all cores)"
    )
    ...
    try:
        ...
        print("Full results saved to: user_data/hyperopt_results_3y.json")
        print("\n" + "=" * 70)
