...
try:
    ...
    from src.risk.circuit_breaker import CircuitBreaker, CircuitBreakerConfig, TripReason
    import src.risk.hrp as hrp
...
class StoicEnsembleStrategyV4(IStrategy):
    ...
    entry_prob_percentile = IntParameter(60, 90, default=75, space='buy')
    entry_prob_window = IntParameter(100, 1000, default=500, space='buy')
    ...
    def __init__(self, config: dict) -> None:
        ...
        # HRP State (Async)
        self._hrp_weights = {}
        self._last_hrp_update = None
        self._hrp_refresh_interval = 60 # minutes
        self._hrp_future: Optional[Future] = None
        self._executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix="hrp_worker")
        ...
        if USE_CUSTOM_MODULES:
            ...
            self.circuit_breaker.initialize_session(initial_balance=0.0) # Will update later
            logger.info("CircuitBreaker initialized and wired.")
    ...
    def bot_start(self, **kwargs) -> None:
        ...
        if self.dp:
            try:
                # Pre-load models for all whitelisted pairs
                # usage of current_whitelist requires the bot to be in running state or at least configured
                pairs = self.dp.current_whitelist()
                logger.info(f"Pre-loading models for {len(pairs)} pairs...")
                for pair in pairs:
                    self._get_pair_model_and_fe(pair)
            ...
            if hasattr(ml_model, 'feature_names_in_'):
                ...
                if missing_features:
                    ...
                    if not getattr(self, '_logged_missing_features_' + pair, False):
                        logger.error(f"Missing features for {pair}: {missing_features[:5]}... Total: {len(missing_features)}. Aborting ML prediction.")
                        setattr(self, '_logged_missing_features_' + pair, True)
                        
                    # CRITICAL FIX: Do not guess 0 for missing features. It introduces noise.
                    # Return neutral prediction instead.
                    dataframe['ml_prediction'] = 0.5
                    dataframe['ml_confidence'] = 0.5
                    dataframe['ml_signal'] = 0
                    return dataframe
    ...
    def _update_regime(self, dataframe: DataFrame) -> None:
        ...
        try:
            ...
            # Store regime score in dataframe for vectorized backtesting
            dataframe['regime_score'] = regime_data['regime_score']
            ...
            if 'regime_score' not in dataframe.columns:
                dataframe['regime_score'] = 50.0  # Default neutral
    ...
    def confirm_trade_entry(self, pair: str, order_type: str, amount: float, rate: float,
        ...
        """
        Final check before entering a trade.
        Uses CircuitBreaker to block trades if risk limits are breached.
        """
        if self.circuit_breaker:
            if not self.circuit_breaker.can_trade():
                logger.warning(f"Trade blocked by CircuitBreaker! State: {self.circuit_breaker.state}")
                return False
                
            # Update balance on first trade or periodically (rudimentary sync)
            try:
                current_balance = self.wallets.get_total_stake_amount()
                self.circuit_breaker.update_balance(current_balance)
            except Exception:
                pass
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        2. We require the prediction to be in the Top 25% of recent history (75th percentile).
        3. We clamp the dynamic threshold between 0.55 and 0.75.
        4. We adjust the threshold based on market regime (defensive -> higher threshold).
        """
        ...
        # 1. Calculate Dynamic Threshold (Rolling Percentile)
        # Use Hyperopt parameter for window size
        window = self.entry_prob_window.value
        percentile = self.entry_prob_percentile.value / 100.0  # e.g., 0.75
        
        # Calculate rolling quantile
        # min_periods=window//4 ensures we get values reasonably quickly
        dynamic_threshold = dataframe['ml_prediction'].rolling(
            window=window, 
            min_periods=min(window, 100)
        ).quantile(percentile)
        
        # Fill NaN at start with safe default
        dynamic_threshold = dynamic_threshold.fillna(0.60)
        
        # 2. Apply Bounds (Min 0.55, Max 0.75) as per Plan
        dynamic_threshold = dynamic_threshold.clip(lower=0.55, upper=0.75)
        
        # 3. Regime Adjustment (Vectorized)
        if 'regime_score' in dataframe.columns:
            # Bearish / Defensive (Score <= 40): Raise threshold (harder to buy)
            # We want max(dynamic_threshold, 0.65)
            defensive_mask = dataframe['regime_score'] <= 40
            dynamic_threshold = np.where(
                defensive_mask, 
                np.maximum(dynamic_threshold, 0.65), 
                dynamic_threshold
            )
            
            # Bullish / Aggressive (Score > 70): Lower threshold (easier to buy)
            # We want max(0.50, dynamic_threshold * 0.9)
            aggressive_mask = dataframe['regime_score'] > 70
            dynamic_threshold = np.where(
                aggressive_mask,
                np.maximum(0.50, dynamic_threshold * 0.9),
                dynamic_threshold
            )
        
        # 4. Volatility Filter
        # ATR > 0.5% of price OR ADX > 25 (Strong trend)
        volatility_filter = (
            (dataframe['atr'] > dataframe['close'] * 0.005) |
            (dataframe['adx'] > 25)
        )

        # 5. Final Entry Condition
        entry_condition = (
    ...
    def _update_hrp_weights(self, current_time: datetime) -> None:
        """Update HRP weights periodically (Async)."""
        # 1. Check if a previous calculation finished
        if self._hrp_future and self._hrp_future.done():
            try:
                new_weights = self._hrp_future.result()
                if new_weights:
                    self._hrp_weights = new_weights
                    self._last_hrp_update = datetime.now()
                    logger.info(f"HRP weights updated (Async): {self._hrp_weights}")
            except Exception as e:
                logger.error(f"Async HRP calculation failed: {e}")
            finally:
                self._hrp_future = None
        
        # 2. Skip if calculation is already running
        if self._hrp_future is not None:
            return

        # 3. Skip if not enough time has passed
        if (self._last_hrp_update and 
            (current_time - self._last_hrp_update).total_seconds() < self._hrp_refresh_interval * 60):
            return

        try:
            # Get all active pairs
            active_pairs = self.dp.current_whitelist()
            ...
            for pair in active_pairs:
                # Use get_pair_dataframe for compatibility with backtest/live
                hist = self.dp.get_pair_dataframe(pair, self.timeframe)
                if not hist.empty:
                    # Use last 500 candles for correlation
                    prices_dict[pair] = hist['close'].tail(500)
            ...
            # Calculate weights (Submit to Executor)
            if USE_CUSTOM_MODULES:
                self._hrp_future = self._executor.submit(hrp.get_hrp_weights, prices_df)
                logger.info("Submitted HRP calculation to background thread")

        except Exception as e:
            logger.error(f"Error triggering HRP update: {e}")

    def custom_stake_amount(self, pair: str, current_time: datetime, current_rate: float,
                           proposed_stake: float, min_stake: Optional[float], 
                           max_stake: float, leverage: float, entry_tag: Optional[str],
                           side: str, **kwargs) -> float:
        """Custom position sizing based on HRP and market regime."""
        
        # 1. Update HRP weights if needed
        self._update_hrp_weights(current_time)
        
        # 2. Get base stake from HRP
        total_balance = self.wallets.get_total_stake_amount()
        weight = self._hrp_weights.get(pair)
        
        if weight:
            # Allocate based on HRP weight, but cap at 30% to prevent over-concentration
            # and respect max_open_trades logic implicitly via Freqtrade
            stake = total_balance * min(weight, 0.30)
        else:
            # Fallback to proposed stake (equal distribution)
            stake = proposed_stake
        
        # 3. Adjust based on market regime (Overlay on HRP)
