...
class NightlySharpeRatioObjective:
    ...
    def __init__(self, X: pd.DataFrame, y: pd.Series, close_prices: pd.Series, 
                 n_splits: int = 5, resource_monitor: Optional[ResourceMonitor] = None):
        """
...
class NightlyHyperparameterOptimizer:
    ...
    def optimize(self, 
                 ...
                 n_trials: int = 500,
                 timeout: Optional[int] = 28800,  # 8 hours default
                 n_jobs: int = -1,
        ...
        """
        logger.info(f"Starting nightly hyperparameter optimization")
        logger.info(f"Target: {n_trials} trials, timeout: {timeout}s, jobs: {n_jobs}")
        
        # Create objective function with resource monitoring
        objective = NightlySharpeRatioObjective(X, y, close_prices, n_splits=5, 
...
def main():
    ...
    try:
        ...
        print(f"Report: user_data/nightly_hyperopt/optimization_report.md")
        print("\n" + "="*70)
