...
class NightlyMonitor:
    ...
    def check_new_results(self, hours_threshold: int = 24) -> Tuple[bool, List[Path]]:
        ...
        result_patterns = ["full_results_nightly.json", "best_params_nightly.json", 
                          "optimization_report.md", "intermediate_results_*.json"]
    ...
    def compare_with_previous(self, current_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if previous_sharpe is not None and comparison["current_sharpe"] is not None:
            if abs(previous_sharpe) > 0.0001:  # Avoid division by zero
                improvement_pct = ((comparison["current_sharpe"] - previous_sharpe) / 
                                  abs(previous_sharpe) * 100)
    ...
    def generate_summary(self, results: Dict[str, Any], 
        ...
        significant_changes = {k: v for k, v in comparison["parameter_changes"].items() 
...
def main():
    ...
    parser.add_argument(
        '--check-results',
        action='store_true',
        help='Check for new optimization results'
    )
    parser.add_argument(
        '--send-report',
        action='store_true',
    ...
    parser.add_argument(
        '--telegram',
        action='store_true',
        help='Send report via Telegram (requires credentials)'
    )
    parser.add_argument(
        '--check-data',
        action='store_true',
    ...
    if args.check_results:
        ...
        if has_new:
            ...
            print("⏳ No new results found in the last {} hours".format(args.hours_threshold))
            results_summary["has_new"] = False
    ...
    if args.check_resources:
        ...
        if "error" in resources:
            ...
            print(f"💾 Memory: {resources['memory_used_gb']:.1f}GB ({resources['memory_percent']:.1f}%)")
            print(f"💿 Disk: {resources['disk_free_gb']:.1f}GB free ({resources['disk_percent']:.1f}% used)")
    ...
    if results_summary.get("has_new") and results_summary.get("comparison", {}).get("has_improvement"):
