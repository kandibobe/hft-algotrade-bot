...
class NightlySharpeRatioObjectiveOvernight:
    ...
    def __init__(self, X: pd.DataFrame, y: pd.Series, close_prices: pd.Series, 
                 n_splits: int = 5, resource_monitor: Optional[ResourceMonitorOvernight] = None,
                 n_jobs_xgb: int = 4):
        """
...
class NightlyHyperparameterOptimizerOvernight:
    ...
    def check_data_freshness(self, symbol: str = "BTC/USDT", timeframe: str = "5m", 
                           max_age_hours: int = 24) -> Tuple[bool, Optional[str]]:
        """
        ...
        try:
            ...
            data_file = self.output_dir / "data" / "binance" / f"{pair_filename}-{timeframe}.feather"
    ...
    def optimize(self, 
                 ...
                 n_trials: int = 1000,
                 timeout: Optional[int] = 43200,  # 12 hours default for overnight
                 n_jobs: int = 8,  # Half of 16 cores
                 resume: bool = True,
                 n_jobs_xgb: int = 4) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(f"Starting overnight hyperparameter optimization")
        logger.info(f"Target: {n_trials} trials, timeout: {timeout}s, jobs: {n_jobs}, XGBoost jobs: {n_jobs_xgb}")
        
        # Create objective function with resource monitoring
        objective = NightlySharpeRatioObjectiveOvernight(
            X, y, close_prices, n_splits=5, 
    ...
    def _save_intermediate_results(self, study: optuna.Study):
        ...
        intermediate_path = self.nightly_dir / f"intermediate_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def _train_final_model(self, X: pd.DataFrame, y: pd.Series, 
                          params: Dict, n_jobs_xgb: int = 4) -> Any:
        """
    ...
    def generate_report(self, results: Dict[str, Any]):
        ...
        with open(report_path, 'w') as f:
            ...
            f.write(f"- **Optimization Duration:** {results['resource_stats']['elapsed_hours']:.2f} hours\n\n")
            ...
            if previous_best:
                ...
                for param in sorted(all_params):
                    ...
                    if param in ['learning_rate', 'gamma', 'reg_alpha', 'reg_lambda', 'subsample', 'colsample_bytree']:
            ...
            f.write(f"- **Peak Memory Usage:** {stats['memory_used_gb']:.1f} GB ({stats['memory_percent']:.1f}%)\n")
            ...
            f.write("1. **Model Deployment:** Consider deploying the new parameters if Sharpe Ratio improvement > 5%\n")
            ...
            if previous_results_path.exists():
                try:
                    with open(previous_results_path, 'r') as f:
                        ...
                        improvement_pct = ((results['best_value'] - prev_value) / abs(prev_value) * 100) if prev_value != 0 else 0
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help='Number of jobs for XGBoost (default: 4)'
    )
    parser.add_argument(
        '--no-resume',
        action='store_true',
        help='Do not resume from previous study'
    )
    parser.add_argument(
        '--check-data',
        action='store_true',
        help='Check data freshness before starting'
    )
    parser.add_argument(
        '--max-data-age',
        type=int,
        default=24,
        help='Maximum data age in hours (default: 24)'
    )
    parser.add_argument(
        '--quick',
        action='store_true',
    ...
    try:
        ...
        if args.check_data:
            is_fresh, message = optimizer.check_data_freshness(
                ...
                max_age_hours=args.max_data_age
            )
            if not is_fresh:
        ...
        print(f"Report: user_data/nightly_hyperopt_overnight/optimization_report_overnight.md")
        print("\n" + "="*70)
