...
class LiquidationGuard:
    ...
    def check_trade_safety(
        ...
        if side == "long":
            if stop_loss_price <= liq_price:
                return False, f"Stop Loss ({stop_loss_price:.2f}) is below Liquidation Price ({liq_price:.2f})"
        else:
            if stop_loss_price >= liq_price:
                return False, f"Stop Loss ({stop_loss_price:.2f}) is above Liquidation Price ({liq_price:.2f})"
        ...
        if dist_to_sl > max_safe_dist:
            recommended_sl = entry_price - max_safe_dist if side == "long" else entry_price + max_safe_dist
            return False, (
    ...
    def get_max_safe_leverage(
        ...
        if side == "long":
            ...
            if target_liq <= 0: return 1.0
            
            # Solve for Lev
            term = (target_liq / entry_price) * (1 - mmr)
            inv_lev = 1 - term
            
            if inv_lev <= 0: return 100.0 # High leverage possible
            return 1.0 / inv_lev
