            ...
            if config:
                ...
                if issues:
                    ...
                    if not config.dry_run and any("High leverage" in i for i in issues):
                        logger.error("CRITICAL: High leverage in live trading! Proceed with caution.")
    ...
    def custom_stake_amount(
        ...
        **kwargs
        ...
        try:
            ...
            if current_dd > 0.05:
                dd_factor = 0.5
                logger.info(f"Drawdown Suppression Active (DD={current_dd:.1%}): Sizing reduced by 50%")
            ...
            logger.info(f"Sizing: Bal={current_balance:.0f} ATR={atr_pct:.1%} DD={current_dd:.1%} -> Stake={safe_stake:.2f}")
