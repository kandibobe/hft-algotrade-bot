...
from src.ml.training.model_trainer import TrainingConfig as ModelTrainerConfig
...
class ModelOptimizer:
    ...
    def optimize_model(self, data: pd.DataFrame, pair: str) -> dict[str, Any]:
        ...
        study = optuna.create_study(
            direction="maximize", sampler=TPESampler(seed=42), study_name=f"{pair}_optimization"
        )
        ...
        return {"success": True, "best_params": best_params, "best_score": best_score}
    ...
    def _sample_ml_params(self, trial: optuna.Trial) -> dict[str, Any]:
        ...
        params = {
            ...
            "medium_period": trial.suggest_int("medium_period", 30, 200),
        }
    ...
    def _simulate_trading(
        self, probs: np.ndarray, y_true: pd.Series, params: dict[str, Any]
    ) -> list[float]:
        ...
        for i, prob_arr in enumerate(probs):
            ...
            if buy_prob > threshold:
                if true_label == 1:
                    ...
                    true_label == -1
                ):  # Assuming -1 is Short/Loss in this simplified view or we map it
    ...
    def _save_optimization_results(self, pair, best_params, best_score, trials):
        ...
        results = {
            ...
            "n_trials": len(trials),
            "history": [{"number": t.number, "value": t.value, "params": t.params} for t in trials],
        }
